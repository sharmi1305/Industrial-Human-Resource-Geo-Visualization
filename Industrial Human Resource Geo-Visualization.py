...
@st.cache_data
def load_data():
    # IMPORTANT: Use GitHub file path (NOT C:/Users/... path)
    df = pd.read_csv("ihr_geo_data.csv")
...
workers_col = next(
    (col for col in df.columns if "main" in col.lower() and "worker" in col.lower()),
    None
)

if not workers_col:
    st.error("❌ Worker count column not found! Check your dataset.")
    st.stop()
